...
interface OrganizationRecord {
  ...
  authMethod: "sso" | "otp";
  provider: {
    providerId: IdentityProviderType | "email-otp";
    providerLabel: string;
    protocol: IdentityProtocol | "OTP";
    loginLabel: string;
    mfa: "idp-enforced" | "app-enforced";
  };
}
...
const organizations: OrganizationRecord[] = [
  ...
  },
  {
    ...
    provider: {
      ...
      providerLabel: "Email One-Time Password",
      protocol: "OTP",
      loginLabel: "Send secure login code",
      mfa: "app-enforced",
    },
  },
];

export const permissionsByRole: Record<EnterpriseRole, string[]> = {
  "AP Reviewer": [
    "dashboard:view",
    "exceptions:view",
    "invoice:view",
...
export const platformStore = {
  ...
  startEnterpriseAuth(email: string) {
    ...
    const isOtp = result.organization.authMethod === "otp";
  ...
  verifyOtp(authRequestId: string, otp: string) {
    // Mock OTP verification: accept any 6-digit code for the demo
    if (!otp || otp.length < 6) return null;
    return this.completeEnterpriseAuth(authRequestId);
  },
  ...
  completeEnterpriseAuth(authRequestId: string) {
    ...
    const session: EnterpriseSessionRecord = {
      ...
      assurance: {
        mfa: organization.authMethod === "sso" ? "idp-enforced" : "app-enforced",
        sessionType: "mock-enterprise",
      },
