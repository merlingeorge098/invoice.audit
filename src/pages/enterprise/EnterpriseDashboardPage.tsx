...
function isZeroValue(value: string | number): boolean {
  ...
  return str === "0" || str === "0%" || str === "0.00" || str === "₹0" || str === "₹0.00";
}
...
export default function DashboardPage() {
  ...
  const allZero = data.metrics.every((m) => isZeroValue(m.value));
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 409) {
          toast.error("This workspace already has invoices.");
        } else {
  ...
  return (
    ...
    >
      ...
      {allZero && (
        <Card className="mb-6 rounded-3xl border-0 bg-teal-50">
          <CardContent className="p-8">
            <p className="font-heading text-2xl font-semibold text-teal-900">
              Your workspace is ready — add your first invoices
            </p>
            <p className="mt-2 text-sm text-teal-700">
              Upload PDFs via OCR, import from a CSV spreadsheet, or load sample data to see the full workflow.
            </p>
        ...
        <Card className="rounded-3xl border-border/70 bg-card/90">
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button asChild variant="outline" className="rounded-xl">
                <Link to={paths.exceptions}>View exceptions</Link>
              </Button>
            </div>
          </CardHeader>
